      ...
      <div class="container">
        <div class="row">
          ...
          <div class="col-lg-2 col-md-6 mb-5 mb-lg-0">
            ...
            <?php
              foreach($typesForFooter as $typeObject) {
                echo '<li>';
                  $url = $router->generate(
                    'catalog-type',
                    [
                      'idType' => $typeObject->getId()
                    ]
                  );
                  echo '<a href="' . $url . '" class="text-muted">';
                    echo $typeObject->getName();
                  echo '</a>';
                echo '</li>';
              }
            ?>
          ...
          <div class="col-lg-2 col-md-6 mb-5 mb-lg-0">
            ...
            <ul class="list-unstyled">
              <?php
              // attention $brandObject est un Objet ! (de type Model/Brand)
              foreach($brandsForFooter as $brandObject) {
                // on demande à l'objet brand de nous retourner son "name"
                // génération du lien vers une page de type "Marque"
                  $url = $router->generate(
                    'catalog-brand', // le router à besoin de savoir qu'elle route il doit générer
                    [
                      'idBrand' => $brandObject->getId()  // le router a également besoin de savoir quoi mettre dans la partie variable de l'url. Lorsque l'on regarde la configuration de la route ; nous voyons que la partie s'appelle 'idBrand'
                    ]
                  );
